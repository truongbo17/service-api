...
namespace App\Api\ThirdParty\TikWM;
...
class TikWMApi
{
    ...
    /**
     * @var string $path_api_signature
     * Signature url
     * */
    private string $path_api_signature = "api/service/sign";
     ...
     */
    public function signature(string $method, string $url): array
    {
        try {
            $query = [
                'params'  => $url,
            ...
            $endpoint = build_external_url($this->host, $this->path_api_signature, $query);
    ...
    {
        try {
            $challenge_id = $this->getHashTagDetail(method: 'GET', challenge_name: $challenge_name)['id'];
            $query = [
     ...
     * @throws GuzzleException
     */
    public function getVideoDownload(string $method, string $tiktok_url, int $hd = 1): array
    {
        try {
            if ($hd != 0 && $hd != 1) $hd = 1;
            $query = [
